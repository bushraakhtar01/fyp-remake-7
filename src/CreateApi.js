...
class CreateApi extends React.Component{
    ...
    function ab(){
        return new Promise((resolve, reject) => {
          axios.post('https://pehnaway78.000webhostapp.com/api/post/create.php',formData,config)
          .then((response)=>{
            if(response){
              resolve(response);
      ...
      //     if(response.data.message=="Post Created" && formData!==""){
    ...
    render(){
        const { name, code, brand, color, material, price, type, size_id, category_id,file, image,arrival,sale,quantity}=this.state
        return(
